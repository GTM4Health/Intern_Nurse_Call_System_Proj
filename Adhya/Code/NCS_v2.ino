...
// ESP32 tells the server that it is still alive
// even when no buttons are being pressed.
    ...
    while (tcpClient.available())
    {
      ...
      if (c == '\n')
      {
        ...
        if (response.indexOf("\"AUTHORIZED\"") >= 0)
        {
          ...
          // Start heartbeat timer from the moment
...
void sendEvent(String event)
{
  ...
  // Each JSON packet ends with a newline
...
void loop()
{
  ...
  // This runs regardless of whether the buttons
  // are being used.
  //
  // Therefore:
  // ESP32 ON + unused = ONLINE
  // ESP32 OFF = no heartbeat = eventually OFFLINE
